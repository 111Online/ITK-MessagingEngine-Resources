...
<xsl:function name="functx:adjust-dateTime-to-uk-local-time" as="xs:dateTime">
	<xsl:param name="dateTimeIn" as="xs:dateTime"/>
	<xsl:variable name="adjustGmt" select="adjust-dateTime-to-timezone($dateTimeIn, xs:dayTimeDuration('PT0H'))"/>
	
	<!-- BST starts at 1 a.m UTC on last Sunday in March -->
	<xsl:variable name="bstStarts" select="xs:dateTime(concat(year-from-dateTime($adjustGmt), '-03-', 31 - xs:integer(replace(replace(
	replace(replace(replace(replace(replace(format-date(xs:date(concat(year-from-dateTime($adjustGmt), '-03-31Z')), 
'[Fn, 2-2]'), 'su', '0'), 'mo', '1'), 'tu', '2'), 'we', '3'), 'th', '4'), 'fr', '5'),'sa', '6')),'T01:00:00Z'))" as="xs:dateTime"/>
    
	<!-- BST ends at 1 a.m UTC on last Sunday in October -->
	<xsl:variable name="bstEnds" select="xs:dateTime(concat(year-from-dateTime($adjustGmt), '-10-',     31 - xs:integer(replace(replace(
	replace(replace(replace(replace(replace(format-date(xs:date(concat(year-from-dateTime($adjustGmt), '-10-31Z')), 
'[Fn, 2-2]'), 'su', '0'), 'mo', '1'), 'tu', '2'), 'we', '3'), 'th', '4'), 'fr', '5'),'sa', '6')),'T01:00:00Z'))" as="xs:dateTime"/>
